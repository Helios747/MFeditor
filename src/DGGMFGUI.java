...
public class DGGMFGUI extends JPanel implements ActionListener {
	...
	public static Boolean bDisableFeatures = false;
	private JTextField txtSearchForParameters;
	...
	public DGGMFGUI () {
		...
		try {
			...
			if (!conf.strLoadPath.equals("null"))
			{
	...
	private void setPanels () throws IOException {
		...
		buttonArea.add(txtSearchForParameters);
		txtSearchForParameters.setColumns(10);
		
		rigidArea_1 = Box.createRigidArea(new Dimension(1, 10));
		buttonArea.add(rigidArea_1);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == ifShowDefaults) {
			...
			if (!bDisableFeatures)
			{
				try {
					Config conf = new Config();
					conf.saveConfig("ShowDefaults", Boolean.toString(ifShowDefaults.isSelected()));
			...
			if (!bDisableFeatures)
			{
				try {
					Config conf = new Config();
					conf.saveConfig("ShowLastImportHeader", Boolean.toString(ifShowImpHead.isSelected()));
				} catch (IOException e1) {
				}
			}
